    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (complaintInfo) {
          fullResponse += '\n\n'
          if (complaintInfo.id) {
            fullResponse += `📋 Complaint ID: #${complaintInfo.id.slice(0, 8)}\n`
          }
          if (complaintInfo.status) {
            fullResponse += `📊 Status: ${complaintInfo.status.replace('_', ' ').toUpperCase()}\n`
          }
        ...
        const errorMsg = {
          en: `Sorry, I encountered an error: ${response.data.error || 'Unknown error'}`,
          hi: `क्षमा करें, मुझे एक त्रुटि मिली: ${response.data.error || 'अज्ञात त्रुटि'}`,
          mr: `माफ करा, मला एक त्रुटी आली: ${response.data.error || 'अज्ञात त्रुटी'}`
        }
