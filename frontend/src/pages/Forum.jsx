...
import { ThumbsUp, ThumbsDown, MessageCircle, TrendingUp, Users, AlertCircle, Image as ImageIcon, X } from 'lucide-react'
...
const Forum = () => {
  ...
  const fetchForumData = async () => {
    ...
    setError(null)
    try {
      const response = await axios.get(`${API_BASE_URL}/api/forum/complaint/${complaintId}`)
      ...
      setError(err.response?.data?.error || err.message || 'Failed to fetch forum data')
  ...
  const handleVote = async (voteType) => {
    ...
    setVoting(true)
    try {
      const response = await axios.post(`${API_BASE_URL}/api/forum/vote`, null, {
        params: {
      ...
      if (response.data.success) {
        ...
        const action = response.data.action === 'added' ? 'Vote added' : 'Vote removed'
        // Don't show alert for successful votes, just refresh
      } else {
        const errorMsg = response.data.error || response.data.message || 'Failed to vote'
        alert(errorMsg)
      }
    } catch (err) {
  ...
  const handleImageSelect = (e) => {
    ...
    files.forEach(file => {
      if (file.size > maxSize) {
        alert(t('imageSizeError') || `Image ${file.name} is too large. Maximum size is 5MB.`)
        return
      }
      ...
      setImagePreviews(prev => [...prev, { url: previewUrl, name: file.name, file: file }])
    })
  ...
  const uploadImagesToStorage = async (files) => {
    ...
    for (const file of files) {
      try {
        ...
        alert(t('imageUploadError') || `Failed to upload ${file.name}. Please try again.`)
        throw error
      }
    }
  ...
  const handleSubmitPost = async (e) => {
    e.preventDefault()
    if (!newPost.content.trim() || !newPost.author_name.trim() || !newPost.author_email.trim()) {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/forum/post`, null, { params })
      if (response.data.success) {
        ...
        setNewPost({ author_name: newPost.author_name, author_email: newPost.author_email, content: '' })
        setSelectedImages([])
        setImagePreviews([])
        await fetchForumData()
      } else {
    ...
    <div className="forum">
      <div className="forum-header">
        <div>
          ...
          <p>{t('forumSubtitle') || 'Discuss and vote on this complaint. Similar incidents can upvote to increase priority.'}</p>
        </div>
        {forumData && (
          <div className="forum-stats">
            <div className="stat-badge">
              <ThumbsUp size={18} />
              <span>{forumData.upvote_count || 0} {t('upvotes') || 'Upvotes'}</span>
            </div>
            <div className="stat-badge">
              <MessageCircle size={18} />
              <span>{forumData.post_count || 0} {t('posts') || 'Posts'}</span>
            </div>
            {forumData.community_priority_boost > 0 && (
              <div className="stat-badge priority">
                <TrendingUp size={18} />
                <span>{t('communityBoost') || 'Community Boost'}: {(forumData.community_priority_boost * 100).toFixed(0)}%</span>
              </div>
            )}
          </div>
        )}
      ...
      <div className="forum-voting">
        ...
        <p className="voting-description">
          {t('votingDescription') || 'If you have a similar incident, vote to increase priority. High upvotes can boost urgency!'}
        </p>
        ...
        <div className="voting-buttons">
          ...
          >
            ...
            {forumData && <span className="vote-count">{forumData.upvote_count || 0}</span>}
          </button>
          <button
        ...
        {forumData && forumData.upvote_count >= 10 && (
          <div className="priority-notice">
            <AlertCircle size={16} />
            <span>
              {t('priorityBoostActive') || 'This complaint has high community support! Urgency may be boosted.'}
            </span>
          </div>
        )}
      </div>
      ...
      {/* Similar Complaints */}
      {forumData && forumData.similar_complaints && forumData.similar_complaints.length > 0 && (
      ...
      <div className="forum-section">
        ...
        <form onSubmit={handleSubmitPost} className="post-form">
          ...
          <textarea
            placeholder={t('postPlaceholder') || 'Share your experience or thoughts about this complaint...'}
        ...
        <div className="posts-list">
          {forumData && forumData.posts && forumData.posts.length > 0 ? (
            ...
            <div className="no-posts">
              ...
              <p>{t('noPostsYet') || 'No discussions yet. Be the first to share your experience!'}</p>
            </div>
          )}
        </div>
      </div>
