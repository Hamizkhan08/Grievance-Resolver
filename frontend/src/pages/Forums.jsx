...
import { TrendingUp, ThumbsUp, MessageCircle, Clock, MapPin, AlertCircle, Users } from 'lucide-react'
...
const Forums = () => {
  ...
  const fetchTrendingComplaints = async () => {
    ...
    setError(null)
    try {
      const response = await axios.get(`${API_BASE_URL}/api/forum/trending?limit=20`)
      ...
      setError(err.response?.data?.error || err.message || 'Failed to fetch trending complaints')
    ...
    <div className="forums-page">
      <div className="forums-header">
        <div>
          ...
          <p className="forums-subtitle">
            {t('forumsSubtitle') || 'Discover trending complaints and join discussions. Upvote issues that matter to you.'}
          </p>
        </div>
        <div className="forums-stats">
          <div className="stat-card">
            ...
            <div>
              ...
              <span className="stat-label">{t('trendingComplaints') || 'Trending'}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {trendingComplaints.length === 0 ? (
        <div className="forums-empty">
          ...
          <p>{t('noTrendingComplaintsDesc') || 'Be the first to start a discussion! File a complaint to get started.'}</p>
              ...
              >
                <div className="forum-card-header">
                  <div className="complaint-badges">
                    ...
                    >
                      {complaint.status?.replace('_', ' ').toUpperCase() || 'OPEN'}
                    </span>
                  </div>
                ...
                <div className="forum-card-body">
                  ...
                  <div className="complaint-department">
                    <span className="dept-label">{t('department') || 'Department'}:</span>
                    <span className="dept-name">{complaint.responsible_department || 'N/A'}</span>
                  </div>
                </div>
                  ...
                  <div className="engagement-stats">
                    ...
                    {complaint.community_priority_boost > 0 && (
                      <div className="stat-item priority">
                        <TrendingUp size={16} />
                        <span>+{(complaint.community_priority_boost * 100).toFixed(0)}%</span>
                      </div>
                    )}
                  </div>
